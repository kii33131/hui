...
class GongyiController extends AdminController {
	//分类列表
    ...
    //添加分类
     ...
     public function add(){
       ...
       if(IS_POST){
        ...
        $filename['modified']=time();
        //content
        $filename['content']=$_POST['content'];
      ...
      imagezoom('Uploads/'.$filename['filename'], 'Uploads/'.$info['photo']['savepath'].'meidi'.$info['photo']['savename'], 268, 128, '#FFFFFF');
       imagezoom('Uploads/'.$filename['filename'], 'Uploads/'.$info['photo']['savepath'].'vvc'.$info['photo']['savename'], 126, 107, '#FFFFFF');
